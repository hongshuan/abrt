...
$(function () {
    ...
    $('.nav.navbar-nav li, .breadcrumb li.dropdown').on('mouseover', siteNav.openSub)
                                                    .on('mouseout', siteNav.openSub);
    $('.breadcrumb li.dropdown').on('mouseout', siteNav.openSub);

    $('.nav.navbar-nav li a, .breadcrumb li.dropdown a, .nav.navbar-nav li input[type="search"]')
        .on('focus', function (e) {
        ...
        function($log, $window, stepState, $translate, HTTP_STATUS_CODES) {
            ...
            this.errorHandler = function($scope, callback, errorKey) {
                ...
                var returnFunc = function(response) {
                    ...
                    if(!response || (!response.data && response.headersLength === 0)) {
...
//Contact Us App
angular.module('dtContactUs', ['DriveTest.Email', 'DriveTest.services', 'ngResource', 'ui.router', 'ui.bootstrap', 'ngRoute','ngSanitize', 'ContactUsForm.Firstname', 'ContactUsForm.Lastname', 'ContactUsForm.Comments', 'ContactUsForm.Focus', 'ngCookies', 'ngMessages'])
...
dtBlockBookingApp.service('blockSignupService', ['blockSignupData', '$window', '$http', '$log', '$resource',
    function (blockSignupData, $window, $http, $log, $resource) {
        ...
        this.register = function (successCallback, errorCallback) {
            return _schoolInformation.register(blockSignupData).$promise.then(successCallback).catch(errorCallback);
        };

        this.register = function (successCallback, errorCallback) {
            return $http.post($window.dtServiceEndpoint + "/school", blockSignupData).then(successCallback).catch(errorCallback);
        };

        this.removeToken = function (successCallback, errorCallback) {
            return $http.delete($window.dtServiceEndpoint + "/school/token").then(successCallback).catch(errorCallback);
        };

        this.getSchool = function (success, errorHandler) {
            _schoolInformation.getSchool().$promise.then(success).catch(errorHandler);
        };
...
(function (angular) {
    ...
    function blockSummaryService($window, $resource, blockSignupData) {
        this.getPaymentSummary = function (blockFeesRequest, success, errorHandler) {
            return _bookingFees.check(blockFeesRequest).$promise.then(success).catch(errorHandler);
        };
...
(function(angular) {
    ...
    function blockTimeslotService($window, $log, $resource, stepState, locale) {
        ...
        this.getTimeslots = function(data, successCallback, errorCallback) {
            return _timeslots.check(data).$promise.then(successCallback).catch(errorCallback);
        };
        ...
        this.holdBlock = function(localStartTime, blockItems, successCallback, errorCallback) {
            ...
            return _holdblock.check(data).$promise.then(successCallback).catch(errorCallback);
        };
    ...
    .service('loginService', ['$http', '$window',
        function($http, $window){
            ...
            this.login = function(school, successCallback, errorCallback){
                return $http.post($window.dtServiceEndpoint + "/school/token", school).then(successCallback).catch(errorCallback);
            };
            ...
            this.verifyEmailToken = function (emailToken, email, successCallback, errorCallback) {
                ...
                return $http.post($window.dtServiceEndpoint + "/school/emailToken", emailTokenData).then(successCallback).catch(errorCallback);
            };

            this.recover = function (email, successCallback, errorCallback){

                return $http.post($window.dtServiceEndpoint + "/school/recover", email).then(successCallback).catch(errorCallback);
            };

            this.reset = function (token, successCallback, errorCallback){

                return $http.post($window.dtServiceEndpoint + "/school/reset", token).then(successCallback).catch(errorCallback);
            };
...
(function(angular) {
    ...
    function studentListService($log, $resource, stepState) {
        ...
        this.checkStudentEligibility = function(params, successCallback, errorCallback) {
            ...
            return _studentEligibility.check(data).$promise.then(successCallback).catch(errorCallback);
        };
    }
...
dtBookingApp
    .constant('_', window._)
    .controller('calendarController', ['$scope', '$window', '$filter', '$log', '$state', 'stepState', 'userService', 'scheduleAvailability', 'bookingTimeService', 'appointmentService', 'errorService', 'locale', 'recaptchaService',
    function($scope, $window, $filter, $log, $state, stepState, userService, scheduleAvailability, bookingTimeService, appointmentService, errorService, locale, recaptchaService) {
        $scope.driverInfo = {};
        $log.log('calendarController $state ---> ', $state);
        focusOnElementById('booking-calendar');

        $scope.setTopFocus = function (idName) {
        ...
        $scope.selectDate = function (evt, dateIn) {
            ...
            $scope.sameDayWarning = dateIn.appointments.length > 0;
        ...
        $scope.driverSubmit = function () {
            ...
            if (!errorService.isBlocked($scope) && $scope.driver.selectedDate !== undefined) {
                ...
                recaptchaService.execute({action: 'getTimeSlots'}).then(function(recaptcha_token) {
                    bookingTimeService.getTimeSlots($scope.driver.serviceId, $scope.driver.selectedDate, recaptcha_token, function (response) {
                        ...
                        $scope.dateMatrix = disableDateMatrixForMonth($scope.monthDisplayed, $scope.yearDisplayed);
                        $scope.driver.selectedDate = undefined;
                        $scope.loading = false;
                        enableBookingContainer();
                        if (response.data.statusCode === 600) {
                            setError('booking.error.invalid.recaptcha');
                            returnVal = true;
                        } else {
                            setError();
                        }
                    ...
                    setError('booking.app.message.unsupported');
                });
            }
        };
        ...
        var _getCalendarWithAvailabilitySuccessHandler = function(response) {
            $scope.loading = false;
        ...
        function getCalendarWithAvailability(eligibilityRange) {
            $scope.loading = true;
            recaptchaService.execute({action: 'getCalendarWithAvailability'}).then(function(recaptcha_token) {
                scheduleAvailability.getAvailability(
                    $scope.driver.serviceId,
                    $scope.calculatedDate,
                    recaptcha_token,
                    _getCalendarWithAvailabilitySuccessHandler,
                    errorService.errorHandler($scope, function (response) {
                        var returnVal = false;
                        $scope.monthDisplayed = $scope.calculatedDate.getMonth();
                        $scope.yearDisplayed = $scope.calculatedDate.getFullYear();
        ...
        function disableDateMatrixForMonth(month, year) {
            var dateMatrix = [];
                            ...
                            for (var a = 0; a < $scope.appointments.length; a ++) {
                                ...
                                if(flowName === "booking.new.appointment" ){
                                    if(isTimezoneDifferent(timezone)) {
                                        ...
                                        var appt_year = apptDate.getFullYear();
                                        var appt_month = (apptDate.getMonth()) + 1;
                                        appt_month = (appt_month < 10 ? "0" : "") + appt_month;
                                        var appt_day = (apptDate.getDate() < 10 ? "0" : "") + apptDate.getDate();
                                        apptDateStr = "" + appt_year + appt_month + appt_day;
                                    }else{
...
dtBookingApp.controller('completeController', ['$scope', '$window', '$log', '$state', 'stepState', 'userService', 'bookingTimeService', 'finalizeBooking',
    function($scope, $window, $log, $state, stepState, userService, bookingTimeService, finalizeBooking) {
        ...
        focusOnElementById('dashboard-link');
...
dtBookingApp.controller('licenceController', ['$scope', '$log', '$state', '$modal', 'userService', 'bookingTimeService', 'stepState', '$translate', 'licenceClassService', 'appointmentService', 'locale', 'errorService', '$filter', 'registrationSvc',
    function ($scope, $log, $state, $modal, userService, bookingTimeService, stepState, $translate, licenceClassService, appointmentService, locale, errorService, $filter, registrationSvc) {
        ...
        $scope.loading = false;
        enableBookingContainer();
        focusOnElementById('booking-licence');
...
dtBookingApp.controller('locationController', ['$scope', '$rootScope', '$window', '$log', '$state', 'userService', 'bookingTimeService', 'stepState', 'dtcLocationData', 'errorService', 'leafletData', 'leafletBoundsHelpers','dtCacheServices', 'dtCacheServicesKeys',
    function ($scope, $rootScope, $window, $log, $state, userService, bookingTimeService, stepState, dtcLocationData, errorService, leafletData, leafletBoundsHelpers, dtCacheServices, dtCacheServicesKeys) {
        ...
        focusOnElementById('booking-location');
...
dtBookingApp.controller('loginController', ['$scope','$log','$state', '$window','userService', 'stepState', '$routeParams', 'locale','loginSvc', 'errorService','$location', 'recaptchaService',
function($scope,$log,$state,$window,userService,stepState,$routeParams,locale, loginSvc, errorService, $location, recaptchaService) {
    $scope.driverInfo = {};
    $scope.locale = locale;
    ...
    var _onLoginError = errorService.errorHandler($scope, function(response){
        ...
        switch(response.status){
            ...
            case 410:
                ...
                break;
            case 415:
                if (angular.isDefined(response.data.errorMessage) && response.data.errorMessage.contains('Suspicious Action')) {
                    messageKey = 'booking.error.invalid.recaptcha';
                } else {
    ...
    $scope.driverSubmit = function(){
        ...
        if ($scope.token) {
            ...
            recaptchaService.execute({action: 'driverLogin'}).then(function(recaptcha_token) {
                loginSvc.login(user, recaptcha_token, _onLoginCompleted, _onLoginError);
            }).catch(_onLoginError);
        }
    };
...
dtBookingApp.controller('registrationController', ['$scope','$log','$state', '$window', 'userService', 'stepState', '$routeParams', 'locale','registrationSvc', 'errorService', 'recaptchaService',
function($scope,$log,$state,$window,userService,stepState,$routeParams, locale, registrationSvc, errorService, recaptchaService) {
    $scope.driverInfo = {};
    $scope.locale = locale;
    ...
    var _onRegistrationCompleted = function(result) {
        result = result.data;
        $scope.loading = false;
        if(result.registered) {
            stepState.changeState('booking.verify');
    ...
    var _onRegistrationError = errorService.errorHandler($scope, function(response){
        $scope.loading = false;
        var messageKey, returnVal = false;
        $scope.driverInfo.form_submitted = false;
    ...
    $scope.driverSubmit = function(){
        ...
        $scope.invalid_license_info = false;
        $scope.loading = true;
        recaptchaService.execute({action: 'driverRegister'}).then(function(recaptcha_token) {
            registrationSvc.register(user, recaptcha_token, _onRegistrationCompleted, _onRegistrationError);
...
dtBookingApp
    .constant('_', window._)
    .controller('timeslotController', ['$scope', '$filter','$window', '$log','$state', 'userService', 'stepState', 'bookingTimeService', 'appointmentService','locale', 'bookingFees', 'errorService', 'appointmentCancelService', 'recaptchaService',
    function($scope, $filter, $window, $log, $state, userService, stepState, bookingTimeService, appointmentService, locale, bookingFees, errorService, appointmentCancelService, recaptchaService) {
        $scope.locale = locale;
        $scope.locationInfo = {};
        ...
        $scope.loading = false;
        enableBookingContainer();
        focusOnElementById('booking-timeslot');
        $scope.errMsg = errorService.clean($scope);
        ...
        var _refreshTimeSlots = function(user, onSuccess) {
            ...
            recaptchaService.execute({action: 'refreshTimeSlots'}).then(function(recaptcha_token) {
                bookingTimeService.getTimeSlots(user.serviceId, user.selectedDate, recaptcha_token, function(response) {
                    enableBookingContainer();
                    $scope.loadingData = false;
                    user.timeslots = response.availableBookingTimes;
...
angular.module('dtBooking')
    .service('appointmentService', ['$log', '$window', '$resource', '$http', 'userService', function($log, $window, $resource, $http, userService) {
        ...
        this.with48hours = function(appointmentId, success, errorHandler) {
            ...
            return _within48hours.get({guid: appointmentId}).$promise.then(success).catch(errorHandler);
        };
        ...
        this.pendingAppointments = function(success, errorHandler, finalfn) {
            ...
            return _pendingAppointments.get({iecachebuster: new Date().getTime()}).$promise.then(function(result) {
                ...
                success(result);
            }).catch(errorHandler).finally(finalfn);
        };

        this.cancelAppointment = function(appointmentId, success, errorHandler) {
            $log.log('appointmentService.cancelAppointment');
            return _cancelAppointment.cancel({guid: appointmentId}).$promise.then(success).catch(errorHandler);
        };
        ...
        this.holdAppointment = function(serviceId, time, licenceClass, endorsement, frenchTestRequested, success, errorHandler) {
            ...
            }).$promise.then(success).catch(errorHandler);
        };
...
dtBookingApp.service('licenceClassService', ['$resource', '$log', '$window',
    function ($resource, $log, $window) {
        ...
        this.loadLicenceClasses = function (success, errorHandler) {
            ...
            return _licenceClasses.check({iecachebuster: new Date().getTime()}).$promise.then(success).catch(errorHandler);
        };
    }
]);
...
dtBookingApp.service('bookingTimeService', ['$resource', '$log', '$filter', '$window', 'locale', 'userService',
    function ($resource, $log, $filter, $window, locale, userService) {
        ...
        this.getTimeSlots = function (serviceId, date, captcha, success, errorHandler) {
...
dtBookingApp.service('scheduleAvailability', ['$resource', '$log', '$window',
    function ($resource, $log, $window) {
        ...
        this.getAvailability = function (serviceId, date, captcha, success, errorHandler) {
...
dtBookingApp.service('prepareBookingPaymentService', ['$resource', '$window', 'locale', function ($resource, $window, locale) {
    ...
    this.preparePayment = function (appointmentGuid, bookingFeeRequestData, success, errorHandler) {
        ...
        return _preparePayment.post({guid: appointmentGuid}, bookingFeeRequestData).$promise.then(success).catch(errorHandler);
    };
}]);
...
dtBookingApp.service('loginSvc', ['$window', '$http', function ($window, $http) {
    this.login = function (user, captcha, successCallback, errorCallback) {
        ...
        return $http.post($window.dtServiceEndpoint + "/driver/token", registrationData).then(successCallback).catch(errorCallback);
    };
    ...
    this.verify = function (user, successCallback, errorCallback) {
        ...
        return $http.post($window.dtServiceEndpoint + "/driver/email/token", loginData).then(successCallback).catch(errorCallback);
    };
    ...
    this.activate = function (user, successCallback, errorCallback) {
        ...
        return $http.post($window.dtServiceEndpoint + "/driver/emailToken", data).then(successCallback).catch(errorCallback);
    };
    ...
    this.verifyEmailToken = function (emailToken, email, successCallback, errorCallback) {
        ...
        return $http.post($window.dtServiceEndpoint + "/driver/emailToken", emailTokenData).then(successCallback).catch(errorCallback);
    };
}]);
'use strict';

var dtRecaptchaModule = angular.module('DriveTest.services');

dtRecaptchaModule.provider('recaptchaService', function () {
    var onLoadFunctionName = 'recaptchaApiOnloadCallback';

    this.$get = ['$rootScope', '$window', '$q', '$document', '$interval', function ($rootScope, $window, $q, $document, $interval) {
        var deferred = $q.defer(), promise = deferred.promise, recaptcha;

        $window.recaptchaApiLoadedCallback = $window.recaptchaApiLoadedCallback || [];
        ...
        $window.recaptchaApiLoadedCallback.push(callback);

        $window[onLoadFunctionName] = function () {
            $window.recaptchaApiLoadedCallback.forEach(function (callback) {
                callback();
            });
        };
        ...
        function validateRecaptchaInstance() {
            if(!recaptcha) {
                deferred.reject({status: 415, data: 'reCaptcha_not_loaded!'});
            }
            return !!recaptcha;
        }
        ...
        // Check if grecaptcha.execute is not defined already.
        if (isExecuteFunctionAvailable()) {
            callback();
        ...
        return {
            ...
            execute: function (action) {
                if (validateRecaptchaInstance()) {
                    return getRecaptcha().then(function (recaptcha) {
                        return recaptcha.execute('6LepppAUAAAAAFSdZeMq7PMYL_9ydntPgTsVLukM', action);
                    });
                } else {
                    return getRecaptcha();
                }
...
dtBookingApp.service('registrationSvc', ['$window', '$http', '$resource', function ($window, $http, $resource) {
    this.register = function (user, captcha, successCallback, errorCallback) {
        ...
        return $http.post($window.dtServiceEndpoint + '/driver/email', registrationData).then(successCallback).catch(errorCallback);
    };
...
dtBookingApp.service('userService', ['$window', '$resource', '$log', 'user', '$rootScope', 'registrationSvc', '$filter',
    ...
    this.getEligibility = function (success, errorHandler) {
        ...
        _eligibility.check({licenceClass:user.licenceClass, endorsement: user.endorsement, motorcycleType: user.component, checkSameLicense: user.checkSameLicense}).$promise.then(success).catch(errorHandler);
    };
    ...
    this.getDriverInformation = function(success, errorHandler) {
        _driverInformation.getName().$promise.then(success).catch(errorHandler);
    };
...
dtContactUsApp.controller('contactUsController', ['$scope', '$state', '$http', '$window', 'recaptchaService', '$log',
    function ($scope, $state, $http, $window, recaptchaService, $log) {
        $scope.contactUsForm = {};
        ...
        $scope.contactUsFormSubmit = function () {
            ...
            recaptchaService.execute({action: 'contactUs'}).then(function (recaptcha_token) {
                var data = JSON.stringify({
                    ...
                    captchaResponse: recaptcha_token
                });
                $http.post($window.dtServiceEndpoint + '/contactus', data).then(
                    function (result) {
                        if (result.data.statusCode === 200) {
                            ...
                            $state.go('contact-us-success', {'success': 'recaptcha-false'});
                        } else {
                            $scope.showCaptchaFailMsg = false;
                            $state.go('contact-us-success', {'success': 'false'});
                        }
...
function($scope,$state,$stateParams,$http) {
    ...
    }else if($stateParams.success === 'recaptcha-false') {
        $scope.emailResultSuccess = false;
